...
- ⚙️ **パラメータセット管理** - LLMパラメータをテンプレートとして管理・再利用
...
- [x] Docker 化と動作確認
- [x] modes → psets_template/psets_current 設計への移行
...
- セッション編集（タイトル・パラメータセット変更）
- エクスポート（JSON形式、日本語ファイル名対応）
- インポート（閲覧モード）

### パラメータセット管理
- テンプレートの作成・編集・コピー・無効化
- ドラッグ＆ドロップで表示順変更
- LLMパラメータ（model, system_prompt, temperature, top_p, max_tokens, context_messages）をテンプレートとして管理
- セッションごとにパラメータセットを保持・変更可能
